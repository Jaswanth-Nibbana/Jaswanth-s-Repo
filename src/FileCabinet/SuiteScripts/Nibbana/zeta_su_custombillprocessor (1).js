...
* @NModuleScope Public
*/
define(["N/plugin", "N/ui/serverWidget", "N/query"], function (plugin, serverWidget, query) {
    function onRequest(context) {
        try {
            ...
            // Add File Upload Field for CSV with Transaction Numbers
            var csvUploadField = form.addField({
                id: 'custpage_zeta_csv_upload',
                type: serverWidget.FieldType.FILE,
                label: 'Upload CSV File with Transaction Numbers',
            });

            csvUploadField.setHelpText({
                help: 'Upload a CSV file with transaction numbers. First column should contain transaction numbers with "transaction number" as the header. The file will be processed to select matching rows in the sublist.'
            });
            ...
            //  Function to parse CSV content and extract transaction numbers
            function parseCSVFile(csvContent) {
                try {
                    ...
                    if (!headerRow.includes('transaction number')) {
                        throw new Error('First column header must be "transaction number"');
                    }
                    for (var i = 1; i < lines.length; i++) {
                        var line = lines[i].trim();
                        if (line) {
                            var columns = line.split(',');
                            var trannumber = columns[0].trim().replace(/"/g, '');
                            if (trannumber) {
                                tranNumbersArr.push(trannumber);
                            }
                        }
                    }
                    if (tranNumbersArr.length === 0) {
                        throw new Error('No transaction numbers found in the CSV file');
                    }
                    console.log('Parsed transaction numbers from CSV:', tranNumbersArr);
                    return tranNumbersArr;
                } catch (error) {
            ...
            // --- NEW: Fetch internal IDs from backend using SuiteQL ---
            function fetchInternalIdsFromTranNumbers(tranNums, fileName, callback) {
                var suiteletUrl = 'https://3388294-sb1.app.netsuite.com/app/site/hosting/scriptlet.nl?script=2324&deploy=1'
                var payload = {
                    action: 'lookupInternalIds',
                    transactionNumbers: tranNums
                };
                fetch(suiteletUrl, {
                    method: 'POST',
                ...
                .then(data => {
                    if (data.success && Array.isArray(data.internalIds)) {
                        callback(null, data.internalIds);
                    } else {
                        callback(new Error(data.message || 'Failed to fetch internal IDs'));
                    }
                })
                .catch(err => {
                    callback(err);
            ...
            function handleFileUpload(event) {
                ...
                function processCsvData() {
                    ...
                    reader.onload = function(e) {
                        ...
                        var allTranNumbers = parseCSVFile(csvContent);
                        if (allTranNumbers && allTranNumbers.length > 0) {
                            fetchInternalIdsFromTranNumbers(allTranNumbers, file.name, function(err, internalIds) {
                                if (err) {
                                    alert('Error fetching internal IDs: ' + err.message);
                                    return;
                                }
                                ...
                                if (isNewFile) {
                                    ...
                                    initializeGlobalTracker && initializeGlobalTracker(file.name, internalIds);
                                    alert('CSV processed: ' + file.name + ' with ' + internalIds.length + ' IDs. Applying selections to current page...');
                                } else {
